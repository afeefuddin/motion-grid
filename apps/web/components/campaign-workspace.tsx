...
function ApprovalView({ approvals, replay, onDecided }: Readonly<{ approvals: MessageApproval[]; replay: boolean; onDecided: (approvalId: string, status: "approved" | "rejected") => void }>) {
...
function ApprovalButtons({ approval, replay, onDecided }: Readonly<{ approval: MessageApproval; replay: boolean; onDecided: (approvalId: string, status: "approved" | "rejected") => void }>) {
  ...
  async function decide(approved: boolean) {
    ...
    try {
      await readContractJson(await fetch(`/api/messages/${approval.messageId}/approve`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ approved, decidedBy: "demo-operator" }) }));
